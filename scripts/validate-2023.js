...
console.log('=== VALIDATING 2023-2024 CANDIDATES ===\n');
    ...
    https.get(`https://api.themoviedb.org/3/tv/${c.tmdbId}?api_key=ca9b21cb89de2d1debed1050f603d7ad`, res => {
      ...
      res.on('end', () => {
        try {
          ...
          const year = parseInt((j.first_air_date || '2023').substring(0, 4));
          ...
          const isAnime = genres.some(g => g.includes('animation')) || 
                         j.name.toLowerCase().includes('frieren') ||
                         j.name.toLowerCase().includes('oshi') ||
                         j.name.toLowerCase().includes('renegade immortal') ||
                         j.name.toLowerCase().includes('apothecary');
          const isReality = genres.some(g => g.includes('reality'));
          const hasEnoughEps = eps >= 4 && eps < 500;
          ...
          if (isDrama && !isAnime && !isReality && hasEnoughEps && notInPool && isRecent) {
            ...
            console.log('✓ ' + j.name + ' (' + year + ') - ' + eps + ' eps - ' + genres.join(', '));
          } else {
            const reason = isAnime ? 'ANIME' : isReality ? 'REALITY' : !isDrama ? 'NOT DRAMA' : eps < 4 ? 'SHORT' : eps > 500 ? 'LONG' : !notInPool ? 'DUP' : 'SKIP';
            console.log('✗ ' + j.name + ' - ' + reason);
          }
        } catch(e) { console.log('ERR: ' + c.title); }
        checked++;
        if (checked === Math.min(15, q.candidates.length)) {
          console.log('\n=== VALID: ' + valid.length + ' ===');
          valid.forEach((v, i) => console.log((i+1) + '. ' + v.title + ' (' + v.year + ') - ' + v.eps + ' eps'));
