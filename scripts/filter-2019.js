...
console.log('=== FILTERING 2019-2022 DRAMAS ===\n');
    ...
    https.get(`https://api.themoviedb.org/3/tv/${c.tmdbId}?api_key=ca9b21cb89de2d1debed1050f603d7ad`, res => {
      ...
      res.on('end', () => {
        try {
          ...
          const year = parseInt((j.first_air_date || '2020').substring(0, 4));
          ...
          if (isDrama && !isAnime && !isReality && hasEnoughEps && notInPool && isRecent) {
            ...
            console.log('✗ ' + j.name + ' - ' + (isAnime ? 'ANIME' : isReality ? 'REALITY' : !isDrama ? 'NOT DRAMA' : !notInPool ? 'DUP' : 'SKIP'));
          }
        } catch(e) { console.log('ERR: ' + c.title); }
        checked++;
        if (checked === Math.min(20, q.candidates.length)) {
          console.log('\n=== VALID: ' + valid.length + ' ===');
